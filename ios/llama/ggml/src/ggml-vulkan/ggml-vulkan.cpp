...
static vk_device ggml_vk_get_device(size_t idx) {
    ...
    if (vk_instance.devices[idx] == nullptr) {
        ...
        device->physical_device.getFeatures2(reinterpret_cast<vk::PhysicalDeviceFeatures2*>(&device_features2));
        ...
        std::vector<const char*> verified_extensions;
        for (const char* ext : device_extensions) {
            ...
            if (found) {
                verified_extensions.push_back(ext);
            } else {
                GGML_LOG_WARN("ggml_vulkan: WARNING: Extension %s requested but not supported by device, skipping.\n", ext);
            }
        }
        ...
        try {
            ...
            GGML_LOG_ERROR("ggml_vulkan: CRITICAL ERROR: createDevice failed: %s\n", e.what());
            return nullptr;
        }

        if (!device->device) {
            GGML_LOG_ERROR("ggml_vulkan: CRITICAL ERROR: createDevice returned null\n");
            return nullptr;
        }
...
static void ggml_vk_print_gpu_info(size_t idx) {
    ...
    vk::PhysicalDevice(physical_device).getFeatures2(reinterpret_cast<vk::PhysicalDeviceFeatures2*>(&device_features2));
...
static bool ggml_vk_device_is_supported(const vk::PhysicalDevice & vkdev) {
    ...
    vk::PhysicalDevice(vkdev).getFeatures2(reinterpret_cast<vk::PhysicalDeviceFeatures2*>(&device_features2));
